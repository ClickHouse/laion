...
FROM system.columns
WHERE (table IN ('laion_100m', 'laion_1m', 'laion_10m')) AND (name IN ('text_embedding', 'image_embedding'))
GROUP BY
    table,
    name
ORDER BY table DESC
...
│ laion_100m │ text_embedding  │ 172.79 GiB      │ 269.12 GiB        │  1.56 │
│ laion_100m │ image_embedding │ 172.76 GiB      │ 269.12 GiB        │  1.56 │
└────────────┴─────────────────┴─────────────────┴───────────────────┴───────┘
...
The compression point of floating point numbers is an area of research, with a number of promising candidates such as X for possible addition to ClickHouse.

Other options include reducing the precision of our floating points to 16bits.
...
| laion_100m | 100m        | 111.857s    | 28.917s    |
| laion_1b   | 1 billion   |             |            |
| laion_2b   | 2 billion   | 3163.470s   | 2730.828   |

As shown, presence of data in the system caches makes a considerable difference on the performance of our linear scans. Performance remains approximately lineart while our compressed data size can fit in RAM. Once this is no longer possible, performance degrades and is I/O bound. This highlights the importance of compression of our floating point data.
...
│ laion_10m           │ text_embedding  │ 18.36 GiB       │ 28.59 GiB         │  1.56 │
│ laion_10m           │ image_embedding │ 18.36 GiB       │ 28.59 GiB         │  1.56 │
│ laion_100m_bfloat16 │ image_embedding │ 127.00 GiB      │ 268.70 GiB        │  2.12 │
│ laion_100m_bfloat16 │ text_embedding  │ 127.30 GiB      │ 268.70 GiB        │  2.11 │
│ laion_100m          │ text_embedding  │ 172.79 GiB      │ 269.12 GiB        │  1.56 │
│ laion_100m          │ image_embedding │ 172.76 GiB      │ 269.12 GiB        │  1.56 │
└─────────────────────┴─────────────────┴─────────────────┴───────────────────┴───────┘
...
This increase in compression has other potential benefits aside from reducing disk space. Below we show the query performance for tables containing 10m and 100m rows, using embeddings encoded as Float32 and bfloat16 (with ZSTD levels 1 and 3). These results are based on the same query as used earlier.
...
The gains on our linear scan speed here are appreciable with the Bfloat16 variant, with ZSTD level 3, improving our performance from Xsecs to Xsecs for the 100m row dataset. An obvious question might be how this reduction in precision impacts our ability to represent real word concepts in our vectors and resulting search quality - we have, afterall, reduced the information encoded in our multi-dimension space and effectively condensed our vectors "closer" together. Below we show the results for the earlier "a sleepy ridgeback dog" query using our new `laion_100m_v2` table and our `search.py` script.

```bash
python search.py --table laion_100m_bfloat16 --query "a sleepy ridgeback dog"
```
...
<functions>
    <function>
        ...
        <name>generateEmbeddingFromText</name>
